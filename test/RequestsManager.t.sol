...
contract RequestsManagerTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory issueTokenInitData =
            abi.encodeWithSelector(SimpleToken.initialize.selector, "Issue Token", "ISSUE");
        ...
        manager = new RequestsManager(address(issueToken), treasury, address(whitelist), allowedTokens);
    ...
    function test_Constructor() public {
        ...
        RequestsManager newManager = new RequestsManager(address(issueToken), treasury, address(whitelist), tokens);
    ...
    function test_RequestMint_Success() public {
        ...
        (
            uint256 id,
            address provider,
            IRequestsManager.State state,
            uint256 amount,
            address token,
            uint256 minExpected
        ) = manager.mintRequests(0);
        assertEq(id, 0);
    ...
    function test_CancelMint_RevertWrongState() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IRequestsManager.IllegalState.selector, IRequestsManager.State.CREATED, IRequestsManager.State.CANCELLED
            )
        );
        manager.cancelMint(0);
    }
