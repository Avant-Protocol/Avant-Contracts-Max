...
contract DeploymentScript is Script {
    ...
    function run() public {
        ...
        address tokenProxy = Upgrades.deployUUPSProxy(
            "SimpleToken.sol", abi.encodeCall(SimpleToken.initialize, (TOKEN_NAME, TOKEN_SYMBOL))
        );
        ...
        address priceStorageProxy = Upgrades.deployUUPSProxy(
            "PriceStorage.sol",
            abi.encodeCall(
                PriceStorage.initialize, (PRICE_UPDATE_UPPER_BOUND_PERCENTAGE, PRICE_UPDATE_LOWER_BOUND_PERCENTAGE)
            )
        );
        ...
        RequestsManager requestsManager =
            new RequestsManager(address(tokenProxy), MINT_DEPOSIT_VAULT, address(addressesWhitelist), inputTokens);
