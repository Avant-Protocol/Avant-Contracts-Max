...
contract PriceStorage is IPriceStorage, AccessControlDefaultAdminRulesUpgradeable {
    ...
    function setPrice(bytes32 _key, uint128 _price) external onlyRole(SERVICE_ROLE) {
        ...
        if (lastPriceValue != 0) {
            uint256 upperBound = uint256(lastPriceValue)
                + ((uint256(lastPriceValue) * upperBoundPercentage) / BOUND_PERCENTAGE_DENOMINATOR);
            uint256 lowerBound = uint256(lastPriceValue)
                - ((uint256(lastPriceValue) * lowerBoundPercentage) / BOUND_PERCENTAGE_DENOMINATOR);
